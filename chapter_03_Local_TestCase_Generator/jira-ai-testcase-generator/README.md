...
your **local Ollama model (Gemma 3 1B)**, with **Groq** as an alternative or fallback provider.
...
- **Local-first AI** — uses your existing Ollama installation (Gemma 3 1B).
...
- **Secure configuration** — credentials live in a gitignored `.env`; secrets are masked in the UI and never logged.
...
- **Python 3.10+**
- **Ollama** installed and running locally with the Gemma 3 1B model pulled.
...
```bash
ollama list          # confirm "gemma3:1b" (or your installed Gemma 3 tag) is present
...
| `OLLAMA_MODEL` | Your installed Gemma 3 tag, e.g. `gemma3:1b` |
...
| `model 'gemma3:1b' is not installed` | Run `ollama list`, then set `OLLAMA_MODEL` to the exact tag shown, or `ollama pull gemma3:1b`. |
...
| Generation is slow on Ollama | Gemma 3 1B on CPU is normal; give it a moment. Reduce ticket size or use Groq. |
